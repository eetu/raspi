...
Optional service — comment the MEMOS dict in group_data/all.py to
retire it. The task then drops into a cleanup branch that stops +
disables the systemd unit and leaves /var/lib/memos (and the `memos`
BW item) untouched, so re-adding the block + redeploying restores the
service.
"""
...
if MEMOS is None:
    systemd.service(
        name="Stop + disable memos (kept on disk for rollback)",
        service="memos",
        running=False,
        enabled=False,
        daemon_reload=True,
