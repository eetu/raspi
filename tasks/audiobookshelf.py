...
This task is opt-in: comment (or remove) the AUDIOBOOKSHELF block in
group_data/all.py to retire ABS. When the import fails the task drops
into a cleanup branch that stops + disables the systemd unit and
leaves /var/lib/audiobookshelf, the BW item, and the Kanidm OIDC
client untouched — so re-adding the block + redeploying restores the
service. Used to phase ABS out in favour of scribe-shelf without
losing the rollback path."""
...
Image={_image}
Network=host
Volume={CIFS["audiobooks"]["mountpoint"]}/audible/books:/audiobooks:ro
